...
const OrgAnalytics: React.FC = () => {
  ...
  const avgScore = Math.round(
    CANDIDATES.reduce((a, c) => a + c.score, 0) / totalCandidates,
  );
  ...
  const buckets = [
    {
      label: "90–100",
      count: CANDIDATES.filter((c) => c.score >= 90).length,
      color: COLORS.green,
    },
    {
      label: "80–89",
    ...
    {
      label: "60–69",
      count: CANDIDATES.filter((c) => c.score < 70).length,
      color: COLORS.red,
    },
  ];
  ...
  const funnel: { label: CandidateStatus; val: number }[] = [
    {
      label: "Pending",
      val: CANDIDATES.filter((c) => c.status === "Pending").length,
    },
    {
      label: "Review",
      val: CANDIDATES.filter((c) => c.status === "Review").length,
    },
    {
      label: "Recommended",
      val: CANDIDATES.filter((c) => c.status === "Recommended").length,
    },
    {
      label: "Declined",
      val: CANDIDATES.filter((c) => c.status === "Declined").length,
    },
  ];
  ...
  const topSkills = Object.entries(skillTotals)
    .map(([label, scores]) => ({
      label,
      score: Math.round(scores.reduce((a, v) => a + v, 0) / scores.length),
    }))
  ...
  return (
    ...
    >
          ...
          >
            {buckets.map((b, i) => {
              ...
              return (
                ...
                >
                  <Typography
                    sx={{ fontSize: 12, fontWeight: 700, color: b.color }}
                  >
                    {b.count}
                  </Typography>
                  <Box
                    sx={{
                      ...
                      background: `linear-gradient(180deg,${b.color},${alpha(b.color, 0.5)})`,
      ...
      <Box
        className="fade-up-3"
        sx={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: "18px" }}
      >
        {/* Role performance */}
        <SoftCard sx={{ p: "26px 28px" }}>
                  ...
                  sx={{
                    ...
                    borderBottom:
                      i < arr.length - 1
                        ...
                        : "none",
                    cursor: "pointer",
                    "&:hover": { background: alpha(COLORS.indigo, 0.02) },
                    borderRadius: "8px",
                    px: "4px",
                  }}
                >
                  <Box sx={{ flex: 1 }}>
                    <Typography
                      sx={{ fontSize: 14, fontWeight: 600, mb: "2px" }}
                    >
                      {r.title}
                    </Typography>
                    <Typography sx={{ fontSize: 12, color: COLORS.textMuted }}>
                      {r.total} candidates · {r.dept}
                    </Typography>
                  </Box>
                  {/* Mini bar */}
                  <Box sx={{ width: 80 }}>
