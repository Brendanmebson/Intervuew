...
const ForTeams: React.FC = () => {
  return (
    <Box sx={{ pt: '64px' }}>
      {/* Hero */}
      <Box sx={{ position: 'relative', py: { xs: '56px', md: '90px' }, px: { xs: '20px', md: '48px' }, overflow: 'hidden', background: 'linear-gradient(180deg,#0A0C10 0%,#0F1115 100%)' }}>
        ...
        <Box sx={{ maxWidth: 740, mx: 'auto', textAlign: 'center', position: 'relative' }}>
          ...
          <Typography variant="h1" sx={{ fontSize: 'clamp(28px,5vw,60px)', mb: '20px', letterSpacing: '-0.03em', color: 'white' }}>
            Hiring at scale,<br />
          ...
          <Typography sx={{ fontSize: { xs: 15, md: 18 }, color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, mb: '40px', maxWidth: 500, mx: 'auto' }}>
      ...
      {/* Use cases */}
      <Box sx={{ maxWidth: 1140, mx: 'auto', px: { xs: '16px', md: '48px' }, py: { xs: '48px', md: '80px' } }}>
        <Box sx={{ textAlign: 'center', mb: '52px' }}>
          <SectionLabel>Use Cases</SectionLabel>
          <Typography variant="h2" sx={{ fontSize: 'clamp(24px,3vw,42px)' }}>Built for every stage of growth</Typography>
        </Box>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2,1fr)', md: 'repeat(3,1fr)' }, gap: '16px' }}>
          {useCases.map((u, i) => (
            <SoftCard key={i} sx={{ p: { xs: '24px 20px', md: '34px 30px' }, border: `1px solid ${alpha(u.color, 0.12)}`, '&:hover': { borderColor: alpha(u.color, 0.3), boxShadow: `0 20px 60px ${alpha(u.color, 0.1)}`, transform: 'translateY(-4px)' } }}>
              ...
              <Typography variant="h5" sx={{ fontSize: { xs: 17, md: 20 }, mb: '10px', letterSpacing: '-0.02em', lineHeight: 1.25 }}>{u.title}</Typography>
              ...
              <Box sx={{ display: 'flex', alignItems: 'baseline', gap: '8px', pt: '20px', borderTop: `1px solid ${alpha(u.color, 0.1)}` }}>
                <Typography sx={{ fontSize: { xs: 32, md: 38 }, fontWeight: 700, letterSpacing: '-0.04em', color: u.color, lineHeight: 1 }}>{u.stat}</Typography>
      ...
      {/* Team features */}
      <Box sx={{ background: alpha(COLORS.indigo, 0.025), borderTop: `1px solid ${alpha(COLORS.indigo, 0.07)}`, borderBottom: `1px solid ${alpha(COLORS.indigo, 0.07)}`, py: { xs: '48px', md: '72px' }, px: { xs: '16px', md: '48px' } }}>
        <Box sx={{ maxWidth: 1140, mx: 'auto' }}>
          <Box sx={{ textAlign: 'center', mb: '48px' }}>
            <SectionLabel>Platform</SectionLabel>
            <Typography variant="h2" sx={{ fontSize: 'clamp(22px,3vw,40px)' }}>Everything teams need</Typography>
          </Box>
      ...
      <Box sx={{ maxWidth: 1140, mx: 'auto', px: { xs: '16px', md: '48px' }, py: { xs: '48px', md: '80px' } }}>
        <Box sx={{ textAlign: 'center', mb: '48px' }}>
          ...
          <Typography variant="h2" sx={{ fontSize: 'clamp(20px,3vw,38px)' }}>Teams that have made the switch</Typography>
        </Box>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3,1fr)' }, gap: '16px' }}>
          {[
            ...
            <SoftCard key={i} sx={{ p: '32px', position: 'relative', '&:hover': { transform: 'translateY(-3px)' } }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', pt: '18px', borderTop: '1px solid rgba(0,0,0,0.06)', flexWrap: 'wrap', gap: '8px' }}>
                <Box>
      ...
      {/* CTA */}
      <Box sx={{ mx: { xs: '16px', md: '48px' }, mb: '80px', borderRadius: '28px', background: 'linear-gradient(135deg,#0F1115,#1a1d2e)', p: { xs: '40px 24px', md: '60px' }, textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <Typography variant="h2" sx={{ fontSize: 'clamp(24px,3.5vw,44px)', color: 'white', mb: '14px', position: 'relative' }}>Ready to scale your team?</Typography>
        <Typography sx={{ fontSize: { xs: 14, md: 16 }, color: 'rgba(255,255,255,0.45)', mb: '34px', position: 'relative' }}>Start free. No credit card needed.</Typography>
        <Box sx={{ display: 'flex', gap: '12px', justifyContent: 'center', position: 'relative', flexWrap: 'wrap' }}>
